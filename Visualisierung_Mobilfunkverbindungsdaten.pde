...
String beginnDate = "01.06.2016 15:00:00"; // Im Format "dd.MM.yyyy HH:mm:ss"
String endDate = "01.09.2016 00:00:00"; // Im Format "dd.MM.yyyy HH:mm:ss"
...
void setup(){  
  ...
  selectInput("Wähle einen Datensatz (Verbindungen_???.csv) aus. Insgesamt sind "+maxFiles+" Datensätze möglich.", "fileSelected");
}
  ...
  if (selection != null){ // Überprüfen, ob eine Datei ausgewählt wurde
   ...
   // Wenn keine Fehler gefunden wurden, Dateipfad speichern
    ...
    if (fileCount > 0){
      ...
      else println("Keine (bereits verarbeiteten) Datensätze ausgewählt. Starte das Programm erneut.");
    }
    else println("Keine (gültigen) Datensätze ausgewählt. Starte das Programm erneut.");
...
void makeTrackpoints(Table[] data){ // Wegpunkte aus Tabellen erstellen
  ...
  if (sumTrackpoints <= 0) println("Error: Keine Trackpoints gefunden. Das könnte daran liegen, dass die Filter falsch eingestellt wurden. Starte das Programm erneut.");
